...
from PyQt6.QtWidgets import *  # Si ya tienes una importación general
...
COLORS = {
    ...
    'surface_hover': '#2D3649',  # Color hover para superficies
    ...
    'accent_dark': '#E55A2B',    # Naranja oscuro para hover
    'accent_light': '#FF8C61',   # Naranja claro para elementos activos
    ...
    'error_dark': '#CC0000',     # Rojo oscuro para hover
    'warning': '#FFC107',        # Amarillo para advertencias
    'success': '#4CAF50',        # Verde para éxito
    'success_dark': '#388E3C',   # Verde oscuro para hover
    'text_secondary': '#8A93A7', # Gris azulado para texto secundario
    'border': '#374151',         # Color para bordes
    'disabled': '#6B7280',       # Color para elementos deshabilitados
    'shadow': 'rgba(0,0,0,0.2)'  # Color para sombras
}
...
class MainWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.buttons_data = [
            ...
            ("🔒 Cerrar Documento", self.cerrar_documento)  # Nuevo botón
        ]
        ...
        user_info = QLabel(f"Email: {self.email}\nRol: {self.user_role}\nDepartamento: {self.departamento}")
    ...
    def agregar_datos(self):
        try:
            ...
            # Agregar campo de destino final antes del PDF
            destino_final_label = QLabel("Destino Final:")
            destino_final_combo = QComboBox()
            
            # Obtener lista de departamentos de la base de datos
            departamentos = DatabaseManager.get_departamentos()
            if departamentos:
                destino_final_combo.addItems(departamentos)
            else:
            ...
            destino_final_combo.setStyleSheet(f"""
                QComboBox {{
                    background-color: {COLORS['surface']};
                    color: {COLORS['text']};
            ...
            form_layout.addRow("Destino Final:", destino_final_combo)

            # Botón para agregar PDF después del destino final
                ...
                try:
                    ...
                    query = """
                        ...
                        (fecha, establecimiento, tipodocumento, nrodocumento, 
                         materia, estado, lugar_actual, archivo_pdf, destino, firma, destino_final)
                    ...
                    valores = [
                        ...
                        "",  # Destino vacío
                        "",  # Firma vacía
                        destino_final_combo.currentText()  # Destino final seleccionado
                    ]
        ...
        try:
            ...
            DatabaseManager.execute_query(
                """INSERT INTO documento(
                    ...
                    nrodocumento, materia, estado, destino, firma
    ...
    def setup_button_visibility(self):
        ...
        role_permissions = {
            ...
            "usuario": ["Consultar Documento", "📬 Recibir Documentos", 
                       "📊 Generar Reporte Actual", "🔒 Cerrar Documento"]
        }
    ...
    def agrupar_datos(self):
        ...
        for i in range(self.tree_widget.topLevelItemCount()):
            ...
            # Obtener informacin completa del documento
            query = """
                SELECT 
    ...
    def eliminar_datos(self):
        try:
            ...
            confirmacion = QMessageBox.question(
                self,
                "Confirmar Eliminación",
                "¿Está seguro de que desea eliminar este documento? Esta acción eliminará también todas las solicitudes relacionadas.",
                QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No,
                QMessageBox.StandardButton.No
            )

            if confirmacion == QMessageBox.StandardButton.Yes:
                # Primero eliminar registros relacionados en solicitudes_documento
                delete_solicitudes = """
                DELETE FROM solicitudes_documento 
                WHERE id_documento = %s
            """
                DatabaseManager.execute_query(delete_solicitudes, (id_documento,))

                # Luego eliminar el documento
                delete_documento = "DELETE FROM documento WHERE id_documento = %s"
                DatabaseManager.execute_query(delete_documento, (id_documento,))
        ...
        try:
            ...
            query = """
                SELECT fecha, establecimiento, tipodocumento, nrodocumento, 
                       materia, destino, estado, lugar_actual, destino_final
                FROM documento 
                WHERE id_documento = %s
            """
            ...
            # Agregar campo de destino final después de los campos existentes
            destino_final_label = QLabel("Destino Final:")
            destino_final_combo = QComboBox()
            departamentos = DatabaseManager.get_departamentos()
            if departamentos:
                destino_final_combo.addItems(departamentos)
                # Establecer el valor actual del destino final
                destino_final_actual = resultado.get('destino_final', '')
                if destino_final_actual in departamentos:
                    destino_final_combo.setCurrentText(destino_final_actual)
            ...
            def guardar_modificacion():
                try:
                    # Construir la consulta de actualización incluyendo destino_final
                    query = """
                        UPDATE documento 
                        SET fecha = %s, establecimiento = %s, tipodocumento = %s,
                            nrodocumento = %s, materia = %s, estado = %s,
                            destino_final = %s
                        WHERE id_documento = %s
                    """
                    ...
                    valores = [
                        ...
                        inputs['materia'].text(),
                        inputs['estado'].text(),
                        destino_final_combo.currentText(),  # Agregar destino final
                        id_documento
                    ]
    ...
    def enviar_documento(self, documento):
        try:
            ...
            query = """
                ...
                WHERE id_documento = %s
            """
            resultado = DatabaseManager.execute_query(query, (documento['id_documento'],))
            
            if resultado and resultado[0]['estado'] == 'Finalizado':
                self.mostrar_mensaje(
                    "Error",
                    "No se puede enviar un documento que ya está finalizado",
                    QMessageBox.Icon.Warning
                )
                return
    ...
    def confirmar_envio(self, documento, departamento_combo, dialog):
        try:
            ...
            if solicitud_existente:
                ...
                # Actualizar documento con estado "Enviado"
                update_doc_query = """
                    UPDATE documento 
                    SET destino = %s,
                        estado = 'Enviado'
    ...
    def procesar_recepcion(self, solicitud, aceptar, dialog):
        try:
            # Crear diálogo de verificación
            verify_dialog = QDialog(self)
            verify_dialog.setWindowTitle("Verificar Identidad")
            verify_dialog.setFixedWidth(400)
            verify_layout = QVBoxLayout(verify_dialog)

            # Campos de verificación
            form_layout = QFormLayout()
            email_input = QLineEdit()
            email_input.setText(self.email)  # Pre-llenar con el email actual
            email_input.setReadOnly(True)  # El email no se puede cambiar
            ...
            verify_layout.addLayout(form_layout)
            ...
            verify_layout.addWidget(button_box)

            # Estilo
            verify_dialog.setStyleSheet(f"""
                QDialog {{
                    background-color: {COLORS['background']};
                }}
                QLabel {{
            ...
            def verificar_credenciales():
                ...
                is_valid, role, dept = DatabaseManager.validate_login(email_input.text(), password_input.text())
                if is_valid:
                    verify_dialog.accept()
                    # Continuar con el proceso de recepción
                    self.completar_recepcion(solicitud, aceptar, dialog)
                else:
                    QMessageBox.warning(
                        verify_dialog,
                        "Error",
            ...
            button_box.accepted.connect(verificar_credenciales)
            button_box.rejected.connect(verify_dialog.reject)

            # Si el usuario cancela la verificación, no continuar
            ...
            self.mostrar_mensaje(
                "Error",
                f"Error en la verificación: {str(e)}",
                QMessageBox.Icon.Critical
            )

    def completar_recepcion(self, solicitud, aceptar, dialog):
        try:
            estado_solicitud = 'aceptado' if aceptar else 'rechazado'
            estado_documento = 'Recepcionado' if aceptar else 'Envio Rechazado'
            motivo = None
            ...
            query = """
                ...
                WHERE email = %s
            """
            resultado = DatabaseManager.execute_query(query, (self.email,))
            nombre_completo = resultado[0]['nombreusuario'] if resultado else self.email
            ...
            DatabaseManager.execute_query(
                update_solicitud, 
                (estado_solicitud, motivo, self.email, solicitud['id_documento'])
            )
            ...
            asunto = f"Documento {info['tipodocumento']} {info['nrodocumento']} - {estado_solicitud.capitalize()}"
            ...
            if aceptar:
                ...
                DatabaseManager.execute_query(
                    update_documento, 
                    (self.departamento, estado_documento, nombre_completo, solicitud['id_documento'])
                )
                ...
                DatabaseManager.execute_query(
                    update_documento, 
                    (estado_documento, nombre_completo, solicitud['id_documento'])
                )
            ...
            self.mostrar_mensaje(
                "Éxito",
                f"Documento {estado_documento.lower()} correctamente",
                QMessageBox.Icon.Information
            )
        ...
        try:
            ...
            query_conteos = """
                SELECT 
                    ...
                    SUM(CASE WHEN estado = 'Envio Rechazado' THEN 1 ELSE 0 END) as envio_rechazado,
            ...
            headers_resumen = [
                ("Documentos Enviados", resultados_conteo['enviados'] or 0),
                ("Documentos Enviados Rechazados", resultados_conteo['envio_rechazado'] or 0),
    ...
    def cerrar_documento(self):
        """Maneja el cierre de documentos"""
        try:
            # Verificar si hay un item seleccionado
            selected_items = self.tree_widget.selectedItems()
            if not selected_items:
                self.mostrar_mensaje(
                    "Error",
                    "Por favor seleccione un documento para cerrar",
                    QMessageBox.Icon.Warning
                )
                return
            ...
            # Verificar que el documento esté en el departamento actual
            query = """
                SELECT lugar_actual, estado 
                FROM documento 
                WHERE id_documento = %s
            """
            resultado = DatabaseManager.execute_query(query, (id_documento,))
            ...
            if documento['lugar_actual'] != self.departamento:
                self.mostrar_mensaje(
                    "Error",
                    "Solo puede cerrar documentos que estén en su departamento actual",
                    QMessageBox.Icon.Warning
                )
                return
            ...
            if documento['estado'] == 'Finalizado':
                self.mostrar_mensaje(
                    "Error",
                    "Este documento ya está cerrado",
                    QMessageBox.Icon.Warning
                )
                return
            ...
            confirmacion = QMessageBox.question(
                self,
                "Confirmar Cierre",
                "¿Está seguro de que desea cerrar este documento? Esta acción no se puede deshacer.",
                QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No,
                QMessageBox.StandardButton.No
            )
            ...
            if confirmacion == QMessageBox.StandardButton.Yes:
                ...
                update_query = """
                    ...
                    SET estado = 'Finalizado',
                        fecha_cierre = CONVERT_TZ(NOW(), 'UTC', 'America/Santiago')
                    WHERE id_documento = %s
                """
                DatabaseManager.execute_query(update_query, (id_documento,))
...
class AdminPanel(QDialog):
    def __init__(self, parent=None):
        super().__init__(parent)
        self.current_admin = parent.email
    ...
    def _init_ui(self):
        ...
        self.requests_btn = QPushButton("🔔 Solicitudes Pendientes")
        self.requests_badge = QLabel("0")
        self.requests_badge.setStyleSheet(f"""
            QLabel {{
                background-color: {COLORS['error']};
                color: {COLORS['text']};
                border-radius: 12px;
        ...
        self.requests_btn.setStyleSheet(f"""
            QPushButton {{
                background-color: {COLORS['primary']};
                color: {COLORS['text']};
    ...
    def _create_table_container(self):
        """Crea el contenedor de la tabla con efectos visuales mejorados"""
        container = QWidget()
        ...
        self.user_table.verticalHeader().setMinimumSectionSize(45)
        ...
        self.user_table.setColumnWidth(2, 220)
        self.user_table.setColumnWidth(3, 120)
        self.user_table.setColumnWidth(4, 130)
        
        # Estilizado con colores corporativos
        ...
        self.user_table.horizontalHeader().setHighlightSections(False)
    ...
    def _create_button_container(self):
        ...
        layout.setSpacing(1)  # Espaciado entre botones
        
        # Botón Nuevo Usuario
        nuevo_usuario_btn = QPushButton("➕ Nuevo Usuario")
        nuevo_usuario_btn.clicked.connect(self.add_user)
        nuevo_usuario_btn.setStyleSheet(f"""
            QPushButton {{
                background-color: {COLORS['primary']};
                color: {COLORS['text']};
                border: 2px solid {COLORS['primary_light']};
        ...
        # nuevo_doc_action = QAction("📄 Nuevo Tipo Documento", self)
        # nuevo_doc_action.triggered.connect(self.load_users)
        
        departamentos_action = QAction("🏢 Panel de Departamentos", self)
        departamentos_action.triggered.connect(self.show_department_manager)
        ...
        layout.addWidget(nuevo_usuario_btn, alignment=Qt.AlignmentFlag.AlignLeft)
        layout.addStretch()
        layout.addWidget(menu_button, alignment=Qt.AlignmentFlag.AlignRight)
    ...
    def load_users(self):
        try:
            ...
            for i, user in enumerate(users):
                ...
                edit_btn.setFixedSize(60, 20)  # Tamaño más pequeño
                edit_btn.setStyleSheet(f"""
                    QPushButton {{
                        background-color: {COLORS['primary']};
                        color: white;
                ...
                delete_btn.setFixedSize(60, 20)  # Tamaño más pequeño
                delete_btn.setStyleSheet(f"""
                    QPushButton {{
                        background-color: {COLORS['error']};
                        color: white;
                ...
                self.user_table.setColumnWidth(self.user_table.columnCount() - 1, 200)  # Ancho reducido
    ...
    def add_user(self):
        """Abre el diálogo de registro para agregar un nuevo usuario"""
        try:
            dialog = QDialog(self)
            ...
            for widget in [username_input, email_input, password_input, confirm_password]:
            ...
            form_layout.addRow("Email:", email_input)
            form_layout.addRow("Contraseña:", password_input)
            form_layout.addRow("Confirmar:", confirm_password)
            form_layout.addRow("Departamento:", dept_combo)
            ...
            cancel_btn.setStyleSheet(button_style.replace(COLORS['primary'], COLORS['error']))
            ...
            if dialog.exec() == QDialog.DialogCode.Accepted:
                ...
                departamento = dept_combo.currentText()
                ...
                try:
                    ...
                    DatabaseManager.execute_query("""
                        INSERT INTO usuario (nombreusuario, email, password_hash, salt, departamento, rol, estado, fecha_solicitud)
                        VALUES (%s, %s, %s, %s, %s, %s, %s, CONVERT_TZ(NOW(), 'UTC', 'America/Santiago'))
                    """, (username, email, password_hash, salt, departamento, "usuario", "aprobado"))
    ...
    def edit_user(self, user):
        ...
        try:
            departamentos = DatabaseManager.execute_query(
                "SELECT nombre_departamento FROM departamento ORDER BY nombre_departamento"
            )
            dept_combo.addItems([d['nombre_departamento'] for d in departamentos])
            dept_combo.setCurrentText(user['departamento'])
        ...
        form_layout.addRow("Confirmar Contraseña:", confirm_password)
        layout.addWidget(form_container)
        ...
        if dialog.exec() == QDialog.DialogCode.Accepted:
            try:
                if password_input.text():
                    ...
                    """, (email_input.text(), dept_combo.currentText(), 
                          role_combo.currentText(), new_password_hash, 
                          new_salt, user['nombreusuario']))
                else:
                    DatabaseManager.execute_query("""
                        UPDATE usuario 
                        SET email = %s, departamento = %s, rol = %s
                        WHERE nombreusuario = %s
                    """, (email_input.text(), dept_combo.currentText(),
                          role_combo.currentText(), user['nombreusuario']))
    ...
    def delete_user(self, user):
        ...
        message = f"¿Está seguro que desea eliminar al usuario <b>{username}</b> ?\n \n Esta acción no se puede deshacer."
        ...
        msg_box.button(QMessageBox.StandardButton.Yes).setText("Eliminar")
    ...
    def show_department_manager(self):
        try:
            # Crear un diálogo para mostrar los departamentos
            dialog = QDialog(self)
            dialog.setWindowTitle("Gestión de Departamentos")
            ...
            title = QLabel("Gestión de Departamentos")
            title.setStyleSheet(f"""
                QLabel {{
                    color: {COLORS['text']};
                    font-size: 28px;
            ...
            # Botón Nuevo
            add_btn = QPushButton("+ Nuevo Departamento")
            add_btn.clicked.connect(self.add_new_department)  # Conectamos al nuevo método
            add_btn.setStyleSheet(f"""
                QPushButton {{
                    background-color: {COLORS['primary']};
                    color: {COLORS['text']};
            ...
            self.table = QTableWidget()  # Hacemos la tabla un atributo de la clase
            self.table.setColumnCount(2)  # Ahora 2 columnas: Nombre y Acciones
            self.table.setHorizontalHeaderLabels(["Departamentos", "Acciones"])
            
            # Configurar el ancho de las columnas
            header = self.table.horizontalHeader()
            header.setSectionResizeMode(0, QHeaderView.ResizeMode.Stretch)
            header.setSectionResizeMode(1, QHeaderView.ResizeMode.Fixed)
            self.table.setColumnWidth(1, 200)  # Ancho fijo para la columna de acciones
            
            self.load_departments()  # Cargar departamentos
    ...
    def load_departments(self):
        """Método para cargar/recargar los departamentos en la tabla"""
        try:
            departments = DatabaseManager.execute_query("""
            ...
            for i, dept in enumerate(departments):
                # Columna del nombre del departamento
                item = QTableWidgetItem(dept['nombre_departamento'])
                item.setTextAlignment(Qt.AlignmentFlag.AlignLeft | Qt.AlignmentFlag.AlignVCenter)
                self.table.setItem(i, 0, item)
                
                # Crear widget para los botones
                action_widget = QWidget()
                action_layout = QHBoxLayout(action_widget)
                ...
                # Botón Editar
                edit_btn = QPushButton("✏️")
                edit_btn.setToolTip("Editar departamento")
                ...
                # Botón Eliminar
                delete_btn = QPushButton("🗑️")
                delete_btn.setToolTip("Eliminar departamento")
                ...
                self.table.setCellWidget(i, 1, action_widget)
    ...
    def add_new_department(self):
        try:
            ...
            icon_label = QLabel("🏢")
            icon_label.setStyleSheet("font-size: 24px; margin-right: 0px;")
            
            title = QLabel("Nuevo Departamento")
            title.setStyleSheet(f"color: {COLORS['text']}; font-size: 18px; font-weight: bold; margin-left: 0px;")
            ...
            name_input.setPlaceholderText("Ingrese el nombre del departamento")
            ...
            if dialog.exec() == QDialog.DialogCode.Accepted:
                department_name = name_input.text().strip()
                
                if not department_name:
                    QMessageBox.warning(self, "Advertencia", "El nombre del departamento no puede estar vacío")
                    return
                ...
                try:
                    ...
                    QMessageBox.critical(self, "Error", f"Error al crear departamento: {str(e)}")
        
        except Exception as e:
            QMessageBox.critical(self, "Error", f"Error al abrir el formulario: {str(e)}")

    def edit_department(self, dept):
        """Método para editar un departamento"""
        try:
            dialog = QDialog(self)
            dialog.setWindowTitle("Editar Departamento")
            dialog.setMinimumWidth(400)
            ...
            # Título
            title = QLabel("✏️ Editar Departamento")
            title.setStyleSheet(f"""
                QLabel {{
                    color: {COLORS['text']};
                    font-size: 18px;
            ...
            name_input = QLineEdit(dept['nombre_departamento'])
            name_input.setStyleSheet(f"""
                QLineEdit {{
                    background-color: {COLORS['surface']};
                    color: {COLORS['text']};
            ...
            for btn, primary in [(cancel_btn, False), (save_btn, True)]:
                btn.setStyleSheet(f"""
                    QPushButton {{
                        background-color: {COLORS['primary'] if primary else COLORS['surface']};
                        color: {COLORS['text']};
                        border: {f"1px solid {COLORS['primary']}" if not primary else 'none'};
                        padding: 8px 20px;
                        border-radius: 4px;
                        font-weight: {'bold' if primary else 'normal'};
                    }}
                    QPushButton:hover {{
                        background-color: {COLORS['primary_dark'] if primary else COLORS['primary']};
                        color: {COLORS['text']};
                    }}
                """)
            ...
            if dialog.exec() == QDialog.DialogCode.Accepted:
                ...
                """, (new_name, dept['id_departamento']))
                
                self.load_departments()
                QMessageBox.information(self, "Éxito", "Departamento actualizado correctamente")
            
        except Exception as e:
            QMessageBox.critical(self, "Error", f"Error al editar departamento: {str(e)}")

    def delete_department(self, dept):
        """Elimina un departamento después de confirmar"""
        nombre = dept['nombre_departamento']
        message = f"¿Está seguro que desea eliminar el departamento de <b>{nombre}</b> ?\n \n Esta acción no se puede deshacer."
        ...
        if msg_box.exec() == QMessageBox.StandardButton.Yes:
            try:
                ...
                QMessageBox.information(
                    self,
                    "Completado",
                    f"Se eliminó el departamento {nombre}"
                )
            except Exception as e:
                QMessageBox.critical(
                    self,
                    "Error", 
                    f"No se pudo eliminar el departamento: {str(e)}"
                )

    def show_establecimientos(self):
        """Método para mostrar la gestión de establecimientos"""
        try:
            dialog = QDialog(self)
            dialog.setWindowTitle("Gestión de Establecimientos")
            dialog.setMinimumWidth(800)
            dialog.setMinimumHeight(600)
            ...
            # Header
            header_layout = QHBoxLayout()
            title = QLabel("🏫 Gestión de Establecimientos")
            title.setStyleSheet(f"""
                QLabel {{
                    color: {COLORS['text']};
                    font-size: 24px;
            ...
            # Botón Nuevo
            add_btn = QPushButton("+ Nuevo Establecimiento")
            add_btn.clicked.connect(lambda: self.add_establecimiento(dialog))
            add_btn.setStyleSheet(f"""
                QPushButton {{
                    background-color: {COLORS['primary']};
                    color: {COLORS['text']};
            ...
            self.table = QTableWidget()
            self.table.setColumnCount(3)
            self.table.setHorizontalHeaderLabels(["Nombre", "Tipo", "Acciones"])
            ...
            QMessageBox.critical(self, "Error", f"Error al abrir establecimientos: {str(e)}")

    def load_establecimientos(self):
        """Cargar establecimientos en la tabla"""
        try:
            establecimientos = DatabaseManager.execute_query("""
                SELECT * FROM establecimiento 
                ORDER BY nombre_establecimiento
            """)
            
            self.table.setRowCount(len(establecimientos))
            for i, est in enumerate(establecimientos):
                # Nombre
                nombre_item = QTableWidgetItem(est['nombre_establecimiento'])
                self.table.setItem(i, 0, nombre_item)
                
                # Tipo
                tipo_item = QTableWidgetItem(est['tipo_establecimiento'])
                self.table.setItem(i, 1, tipo_item)
                ...
                edit_btn = QPushButton("✏️")
                edit_btn.clicked.connect(lambda _, e=est: self.edit_establecimiento(e))
                delete_btn = QPushButton("🗑️")
                delete_btn.clicked.connect(lambda _, e=est: self.delete_establecimiento(e))
            ...
            QMessageBox.critical(self, "Error", f"Error al cargar establecimientos: {str(e)}")

    def add_establecimiento(self, parent=None):
        """Agregar nuevo establecimiento"""
        try:
            dialog = QDialog(parent or self)
            dialog.setWindowTitle("Nuevo Establecimiento")
            dialog.setMinimumWidth(400)
            ...
            # Título
            title = QLabel("🏫 Nuevo Establecimiento")
            title.setStyleSheet(f"""
                QLabel {{
                    color: {COLORS['text']};
                    font-size: 18px;
            ...
            tipo_combo.addItems(["Educación", "Salud", "Administrativo", "Otro"])
            tipo_combo.setStyleSheet(f"""
                QComboBox {{
                    background-color: {COLORS['surface']};
                    color: {COLORS['text']};
                    border: 1px solid {COLORS['primary']};
            ...
            if dialog.exec() == QDialog.DialogCode.Accepted:
                ...
                DatabaseManager.execute_query("""
                    INSERT INTO establecimiento (nombre_establecimiento, tipo_establecimiento)
                    VALUES (%s, %s)
                """, (nombre, tipo))

                self.load_establecimientos()
                QMessageBox.information(self, "Éxito", "Establecimiento agregado correctamente")

        except Exception as e:
            QMessageBox.critical(self, "Error", f"Error al agregar establecimiento: {str(e)}")

    def edit_establecimiento(self, establecimiento):
        """Editar establecimiento existente"""
        try:
            dialog = QDialog(self)
            dialog.setWindowTitle("Editar Establecimiento")
            dialog.setMinimumWidth(400)
            ...
            # Título
            title = QLabel("✏️ Editar Establecimiento")
            title.setStyleSheet(f"""
                QLabel {{
                    color: {COLORS['text']};
                    font-size: 18px;
            ...
            nombre_input = QLineEdit(establecimiento['nombre_establecimiento'])
            tipo_combo = QComboBox()
            tipo_combo.addItems(["Educación", "Salud", "Administrativo", "Otro"])
            tipo_combo.setCurrentText(establecimiento['tipo_establecimiento'])

            for widget in [nombre_input, tipo_combo]:
            ...
            if dialog.exec() == QDialog.DialogCode.Accepted:
                ...
                DatabaseManager.execute_query("""
                    ...
                    SET nombre_establecimiento = %s, tipo_establecimiento = %s
                    WHERE id_establecimiento = %s
                """, (nombre, tipo, establecimiento['id_establecimiento']))

                self.load_establecimientos()
                QMessageBox.information(self, "Éxito", "Establecimiento actualizado correctamente")

        except Exception as e:
            QMessageBox.critical(self, "Error", f"Error al editar establecimiento: {str(e)}")

    def delete_establecimiento(self, establecimiento):
        """Eliminar establecimiento"""
        username = establecimiento['nombre_establecimiento']
        message = f"¿Está seguro que desea eliminar el establecimiento de <b>{username}</b> ?\n \n Esta acción no se puede deshacer."
        ...
        if msg_box.exec() == QMessageBox.StandardButton.Yes:
            try:
                ...
                QMessageBox.information(
                    self,
                    "Completado",
                    f"Se eliminó el establecimiento {username}"
                )
            except Exception as e:
                QMessageBox.critical(
                    self,
...
class PendingRequestsDialog(QDialog):
    def __init__(self, parent=None):
        ...
        self.requests_table.setHorizontalHeaderLabels([
            ...
            "Fecha Solicitud", "Estado", "Acciones"
        ])
        
        # Configurar la tabla con más espacio vertical
        ...
        # Configurar altura de filas - Aumentada
        self.requests_table.verticalHeader().setDefaultSectionSize(60)  # Más espacio vertical
        self.requests_table.verticalHeader().setMinimumSectionSize(60)
        ...
        self.requests_table.setColumnWidth(0, 120)   # Usuario
        self.requests_table.setColumnWidth(2, 120)   # Departamento
        self.requests_table.setColumnWidth(3, 130)   # Fecha
        self.requests_table.setColumnWidth(4, 80)    # Estado
        self.requests_table.setColumnWidth(5, 280)   # Acciones
    ...
    def load_pending_requests(self):
        try:
            ...
            for i, req in enumerate(requests):
                ...
                # Botones de acción más pequeños pero con más espacio vertical
                action_widget = QWidget()
                action_layout = QHBoxLayout(action_widget)
                action_layout.setContentsMargins(5, 8, 5, 8)  # Más margen vertical
                action_layout.setSpacing(10)
                
                approve_btn = QPushButton("✅ Aprobar")
                reject_btn = QPushButton("❌ Rechazar")
                
                for btn, is_approve in [(approve_btn, True), (reject_btn, False)]:
                    btn.setFixedSize(90, 28)  # Botones más pequeños
                    btn.setStyleSheet(f"""
                        QPushButton {{
                            background-color: {COLORS['success'] if is_approve else COLORS['error']};
                            color: white;
                            border: none;
                            border-radius: 3px;
                            font-size: 11px;
                        ...
                        QPushButton:hover {{
                            background-color: {COLORS['success_dark'] if is_approve else COLORS['error_dark']};
                        }}
                        QPushButton:pressed {{
                            background-color: {COLORS['success'] if is_approve else COLORS['error']};
                        }}
...
class EmailNotifier:
    ...
    SMTP_PORT = 587
    SENDER_EMAIL = "notific.corporacionislademaipo@gmail.com"
    APP_PASSWORD = "wvba chxs hese uhwo"
