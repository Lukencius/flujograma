...
class MainWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.buttons_data = [
            ...
            ("📊 Generar Reporte Actual", self.generar_reporte_actual)  # Asegúrate de que el nombre coincida
        ]
    ...
    def generar_reporte_actual(self):
        try:
            ...
            # Hoja 1: Documentos
            ws_documentos = wb.active
            ws_documentos.title = "Reporte de Documentos"

            # Definir encabezados para la hoja de documentos
            headers = [
                "ID", "Fecha", "Establecimiento", "Tipo Doc", 
                "Nro Doc", "Materia", "Destino", "Firma", "Estado"
            ]
            ...
            for col, header in enumerate(headers, 1):
                ws_documentos.cell(row=1, column=col, value=header)
                cell = ws_documentos.cell(row=1, column=col)
            ...
            for row in range(self.tree_widget.topLevelItemCount()):
                ...
                if not item.isHidden():  # Solo exportar elementos visibles
                    ...
                    # Agregar datos a la hoja de documentos
                    for col in range(len(headers)):
                        ws_documentos.cell(row=row+2, column=col+1, value=item.text(col))
                    
                    # Contar estados (columna 8 es la de Estado)
                    estado = item.text(8).lower().strip()  # Aseguramos texto en minúsculas y sin espacios extras
                    ...
                    if 'pendiente' in estado:
                        ...
                        documentos_finalizados += 1
                    elif 'recepcionado' in estado:
                        documentos_recepcionados += 1

            # Ajustar el ancho de las columnas en la hoja de documentos
            for column in ws_documentos.columns:
                max_length = 0
                ...
                ws_documentos.column_dimensions[column_letter].width = adjusted_width

            # Crear hoja de estadísticas
            ws_stats = wb.create_sheet(title="Estadísticas")
            
            # Estilo para los encabezados de estadísticas
            header_style = {
            ...
            estadisticas = [
                ...
                ["Total de Documentos", documentos_totales],
                ["Documentos Pendientes", documentos_pendientes],
                ["Documentos Finalizados", documentos_finalizados],
                ["Documentos Recepcionados", documentos_recepcionados]
            ]

            # Aplicar estilos y agregar datos a la hoja de estadísticas
            for row_idx, row_data in enumerate(estadisticas, 1):
